...
class GaugeSelfTestThread(QThread):
    """Executes a sequential sweep test across all gauges over 3 seconds each."""
    update_signal = pyqtSignal(str, float)
    test_finished = pyqtSignal()

    def run(self):
        # Sequence order: Big 3, then Right-to-Left across rows 1, 2, and 3
        sequence = [
            ("rpm", 0, 6100),
            ("speed", 0, 220),
            ("cool_temp", 50, 120),
        ...
        steps = 30
        step_delay = 3.0 / (steps * 2)  # 3 seconds total per gauge (1.5s up, 1.5s down)

        for gauge_key, min_v, max_v in sequence:
            # Sweep Min -> Max
            for i in range(steps + 1):
                val = min_v + (max_v - min_v) * (i / steps)
                self.update_signal.emit(gauge_key, val)
...
class SquareGaugeBox(QFrame):
    ...
    def __init__(self, title_text, width=140, height=110, accent_color="#ff5500", title_font_size=None, val_font_size=None):
        ...
        self.flash_text_content = ""

        # Dedicated timer for handling exact 250ms warning flash toggles
        self.flash_timer = QTimer(self)
        self.flash_timer.setInterval(250)
        self.flash_timer.timeout.connect(self._toggle_flash_visuals)
    ...
    def _toggle_flash_visuals(self):
        """Flashes the full background red with white text, alternating to black background with red text every 250ms."""
        ...
        self.value.setText(self.flash_text_content)
    ...
    def set_val(self, text_val, state="BASE", flash_text="WARNING", flash_color="#FF0000"):
        ...
        """
        if state == "FLASH":
            self.flash_text_content = flash_text
            if not self.is_flashing:
                self.is_flashing = True
                self.flash_state = False
                self._toggle_flash_visuals()
                self.flash_timer.start()
            else:
                self.value.setText(flash_text)
            return

        # Stop flashing if transitioning back to a non-flash state
        if self.is_flashing:
...
class RaceDashboard(QMainWindow):
    ...
    def __init__(self):
        global btn_fast_toggle, btn_slow_toggle, btn_color_toggle, exit_btn, btn_test_run
        super().__init__()
        ...
        # Self-Test Button placed under EXIT DASH
        btn_test_run = QPushButton("TEST")
        btn_test_run.setFixedWidth(225)
        btn_test_run.setStyleSheet(self.style_off)
        btn_test_run.clicked.connect(self.start_gauge_test)

        # Stack EXIT DASH and TEST vertically under column 0
        left_btn_container = QVBoxLayout()
        left_btn_container.setSpacing(5)
        left_btn_container.addWidget(exit_btn, alignment=Qt.AlignmentFlag.AlignCenter)
        left_btn_container.addWidget(btn_test_run, alignment=Qt.AlignmentFlag.AlignCenter)

        grid_layout.addLayout(left_btn_container, 3, 0, Qt.AlignmentFlag.AlignCenter)
    ...
    def start_gauge_test(self):
        if self.is_testing:
            return
        self.is_testing = True
        btn_test_run.setStyleSheet(self.style_on)

        self.test_thread = GaugeSelfTestThread()
        self.test_thread.update_signal.connect(self.handle_test_update)
        self.test_thread.test_finished.connect(self.finish_gauge_test)
        self.test_thread.start()

    def handle_test_update(self, key, val):
        if key in ["rpm", "speed", "cool_temp", "oil_temp", "fuel_press", "iat"]:
            telemetry_cache[key] = int(val)
        elif key == "fuel_status":
            telemetry_cache[key] = f"{int(val)}%"
        else:
            telemetry_cache[key] = round(val, 1)

    def finish_gauge_test(self):
        self.is_testing = False
        btn_test_run.setStyleSheet(self.style_off)

        # Reset telemetry cache back to default base values so real-time OBD2 polling populates fresh readings
    ...
    def refresh_screen(self):
        ...
        cool = c["cool_temp"]
        if cool >= 110:
            self.gauges["cool_temp"].set_val(f"{cool}°C", state="FLASH", flash_text="WARNING")
        ...
        oil = c["oil_temp"]
        if oil >= 109:
            self.gauges["oil_temp"].set_val(f"{oil}°C", state="FLASH", flash_text="WARNING")
        elif 0 < oil < 95:
            self.gauges["oil_temp"].set_val(f"{oil}°C", state="FLASH", flash_text="NOT WARM")
        elif 105 < oil < 109:
        ...
        ta = c["timing_advance"]
        if ta < 0:
            self.gauges["timing_advance"].set_val(f"{ta}°", state="FLASH", flash_text="RETARDED")
        ...
        v = c["volt"]
        if 0 < v < 12.0:
            self.gauges["volt"].set_val(f"{v}V", state="FLASH", flash_text="LOW V")
        elif 12.0 <= v < 13.5:
